...
- `/next` is written before `/board` and, with no argument, takes every eligible slice in waves, one per subagent at clean context, up to the PR merged by the policy; `HARNESS_AUTOMERGE` goes `on` on Tipoff from the first run. The judge's model is per tier, Sonnet 5 at tier 1 and Opus 5 at tier 2 (4.4, 5.4; ADR-0002, decisions 3 and 5).
...
From 0.35, from the inbox lines of 22 September:

- The prose quotes the board screen as `scripts/board.sh` prints it: a claimed slice is `in progress`, a `gh` that does not answer is `gh not available`, and the plan the backlog has passed is marked `beyond`. The translation of the script had left `/next` and this spec quoting the Italian strings, which a reader looked for on the screen and did not find. 5.7 names the `docs_mode` key of the policy block, the one `/board` reads, where it named a line of the prose. A test fails when an Italian string of the screen is back in `/next`, `/board` or the spec, and when one of the three English strings leaves `board.sh` (4.4, 5.4, 5.7).

From 0.36, from the inbox line of 22 September:

- What starts `/next` is its command, `/next` for every eligible slice and `/next S12` for one, and the English phrases of its description. S46 had taken the Italian trigger out of the description, and the changes of 0.13 in this section, 4.4, 5.4, two steps of the plan and an open question still quoted it, so whoever read them typed a word that started nothing. The steps of the plan keep what was built and no longer name the word. A test fails when the README, a `SKILL.md` or this spec outside its header quotes it again (4.4, 5.4, 9.3).
...
1. `git switch -c slice/S03-osservatore` and an immediate push of the empty branch. **The branch is the claim**: if the push fails because the branch exists, the slice belongs to someone else. No `owner` field to keep in sync, and the claim is atomic because the server makes it. There is no second mark to leave: the `in progress` state is the branch on the remote, `in-progress` leaves `status`, and the PR of a slice does not touch `docs/backlog/`, which sits among the human-merge paths and would cost it the `human-gate` label and the merge by the policy.
...
5. S12: `/next`, the whole board in one command and the model per tier.
6. S13: the skills that do not repair the harness, `docs/inbox.md`, this repo without the verdict hook.
7. Tipoff ready: the three phases of `/harness-init` rerun from the harness's main, `HARNESS_AUTOMERGE=on`, `/slice` on `SPEC-esc-key.md`, `/next` on the whole board up to two PRs merged by the policy. Then the audit in Docket with Fable, `/board`, and `/audit` after the first month.
...
- `/next` headless: is it really needed, or is the interactive one run on the whole board plus the PRs of GitHub's coding agent enough. To be decided after Tipoff.
